...
def build_model(model_name, ckpt_path=None):
    model_mapping = {
        'Dlib-ResNet': dlib_resnet_wrapper.load_model
        # add more mappings as necessary
    }

    if model_name not in model_mapping:
        raise ValueError('Invalid model name')
    model, transforms = model_mapping[model_name]()

    return model, transforms

def apply_filter_on_image(image, output_path: str = None, filter_name: str = "squid_game_front_man")->Image:
...
def apply_filter_on_video(source, output_path: str = None, filter_name: str = "squid_game_front_man")->None:
    ...
    model, simple_transform = dlib_resnet_wrapper.load_model()
